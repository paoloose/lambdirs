...
/* Authorizer to be used in authenticated endpoints */

resource "aws_api_gateway_authorizer" "lambdirs" {
  name            = "lambdirs-${local.env}-authorizer"
  type            = "COGNITO_USER_POOLS"
  identity_source = "method.request.header.Authorization"

  rest_api_id   = aws_api_gateway_rest_api.lambdirs.id
  provider_arns = [aws_cognito_user_pool.lambdirs.arn]
}
